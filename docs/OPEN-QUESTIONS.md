...
| Q-028 | Brand / product colours | The seven juice accent colours for §17a (anor, apelsin, olcha, shaftoli, olma, multifrukt, qulupnay-ananas). Placeholders ship in `@barff/config`; sampling them from the label artwork is not an option because those renders are AI mockups (Q-022). Two values are needed per juice — a light surface needs a darker accent than a near-black one | S12 product pages, S17 hero | OPEN | — |
| Q-029 | Brand | Sign-off on the light-mode palette. Light is a first-class theme (§16a) and its surfaces, borders and shadows were authored here against WCAG rather than derived from the dark ones. Pairs with Q-011 (the brand green itself is still a placeholder) | S07a, all public pages | OPEN | — |
...
| D-012 | Public reads keep the paths `CLAUDE.md` §11 names instead of moving under `/api/v1/public/*` | S11 | §11 is the source of truth for the API surface, and §30 says take the architecture-compatible option. Cacheability is expressed by `Cache-Control`, which is what a CDN keys on anyway — better than a path glob |
| D-013 | Cache invalidation is a per-namespace generation counter, not `SCAN`/`KEYS` or a key index | S11 | `INCR` retires a namespace in one O(1) command; `KEYS`/`SCAN` walk the whole keyspace and block Redis, and a key index is a second structure that can drift |
| D-014 | A cached response's Redis TTL plus browser `max-age` is clamped to stay inside `S3_SIGNED_URL_TTL` | S11 | Public payloads embed signed media URLs. Outliving them serves a valid-looking response whose images all 404 — a caching bug that presents as a storage bug |
| D-015 | Themed tokens reach CSS as custom properties; only `brand`, spacing, radius and the type scale stay literal | S07a | Tailwind compiled the flat hex into utilities, so there was no runtime value for a toggle or a media query to change. The values that do not differ between themes gain nothing from the indirection |
| D-016 | `theme.css` is generated from `theme.ts` and committed, with a test asserting the two agree | S07a | Keeps one source of truth without adding a build step; a token changed without regenerating fails the test instead of shipping a stylesheet that disagrees with the TypeScript |
| D-017 | `system` removes the `data-theme` attribute rather than resolving it to a value | S07a | Resolving once at load freezes the page at whatever the OS said then. Removing it hands the choice back to the media query, so a visitor changing their OS setting with the tab open is followed live |
| D-018 | Components use semantic accent tokens; the raw `brand-*` ramp is banned by test in both `packages/ui` and `apps/web` | S07a | The ramp is one value in both themes, so `bg-brand-500` passes WCAG on dark and fails at 2.6:1 on light — which is what a Lighthouse run in light mode actually found |
| D-019 | The reactive accent drives glow and highlights only, never text, surfaces or borders | S07a | Makes §17a's "AA at every point of the transition" structurally true rather than a claim about animation frames: the things that must stay AA are not the things that move |
| D-020 | Public API response shapes are declared in `@barff/types` and the API's mappers are annotated with them | S12 | Two hand-written copies of a response shape drift silently — the API renames a field, the app still compiles, and the page renders `undefined`, which reads as missing content rather than a bug |
| D-021 | No route-level `loading.tsx` on the public pages; skeletons are `<Suspense>` fallbacks inside a page | S12 | A loading route makes Next stream, and a streamed response sends `200` before `notFound()` can run (soft 404 on every draft URL) and emits `<meta name="description">` into the body instead of the head. Both verified in a browser |
| D-022 | Sections with no data render nothing, rather than a marked placeholder | S12 | §1 forbids inventing company facts. A homepage showing `1000+ MOCK` is worse than a shorter one: the screenshot outlives the placeholder |
